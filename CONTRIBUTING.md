...
- 操作する場面と理由を先に示す
- ローカルとリモートを明確に区別する
- 画面操作だけでなく、操作直後の成功確認を具体的に示す
- 用語・状態判断・次の操作の確認問題を1問ずつ用意する
- 自分で試す課題と、迷いやすいポイントを用意する
- 任意コマンドには実行場所、前提、影響、GUI対応、出力、失敗時の初動を書く
- 初心者向けでも技術的な誤りを入れない
- 画像だけに重要な情報を持たせない

frontmatterには `lesson`、`title`、`description`、`goals`、`duration`、`prerequisite`、`interfaces`、`commandOptional`、`draft` が必要です。追加・変更後は教材構造テストも更新してください。
